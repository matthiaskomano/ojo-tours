...
import { Plus, MapPin as MapIcon, DollarSign, Edit, Trash2, Eye, Home } from "lucide-react";
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 tracking-tight">Properties</h1>
          <p className="text-sm text-gray-500 mt-2">Manage your luxury lodges and accommodations</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {lodges.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-gray-500 mb-6">Get started by creating your first property</p>
            <Link href="/dashboard/admin/lodges/new">
              <Button className="bg-linear-to-r from-[#da8cff] to-[#9a55ff] hover:opacity-90 text-white">
                <Plus className="mr-2 h-4 w-4" />
                Create Property
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Amenities</th>
                  <th className="px-6 py-4 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {lodges.map((lodge) => (
                  <tr key={lodge.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <img src={lodge.image} alt={lodge.name} className="w-16 h-16 rounded-lg object-cover" />
                        <div>
                          <div className="font-medium text-gray-900">{lodge.name}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
