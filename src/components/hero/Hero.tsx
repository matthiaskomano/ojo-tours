...
const Hero = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(
      `/tours?location=${encodeURIComponent(location)}&date=${date}&guests=${encodeURIComponent(guests)}`,
    );
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 bg-[#040C08]">
        ...
        <div className="absolute inset-0 bg-linear-to-b from-[#040C08]/80 via-black/40 to-[#040C08]/90 z-20"></div>
      </div>
      ...
      >
        <motion.span
          ...
          className="text-gold tracking-[0.3em] uppercase text-xs md:text-sm font-bold mb-4 block"
        >
        ...
        >
          Discover Rwanda <br className="hidden sm:block" /> with{" "}
          <span className="text-transparent bg-clip-text bg-linear-to-r from-gold to-white border-b-2 border-gold pb-2">
            OJO Tours
          </span>
        </motion.h1>
        ...
        <motion.p
          variants={itemVariants}
          className="text-base sm:text-lg md:text-xl text-white/80 max-w-2xl mb-10 md:mb-12 font-light px-4"
        >
          Experience gorilla trekking, safaris, luxury vacations, and
        ...
        <motion.div
          variants={itemVariants}
          className="flex flex-col sm:flex-row gap-4 mb-12 md:mb-16 w-full sm:w-auto px-4 sm:px-0"
        >
          <Link
            href="/tours"
            className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-4 rounded-full font-bold tracking-wider uppercase text-sm transition-all duration-300 text-center inline-block w-full sm:w-auto shadow-lg"
          >
            Book a Tour
          </Link>
          <Link
            href="/lodges"
            className="bg-black/30 border border-white/30 hover:border-gold hover:text-gold hover:bg-black/50 text-white px-8 py-4 rounded-full font-bold tracking-wider uppercase text-sm transition-all duration-300 backdrop-blur-md text-center inline-block w-full sm:w-auto shadow-lg"
          >
        ...
        >
          ...
          <div className="flex-1 w-full flex items-center bg-black/40 border border-white/10 rounded-2xl px-4 py-3.5 focus-within:border-gold/50 transition-colors group hover:bg-black/60">
            <MapPin
              className="text-gold mr-3 shrink-0 group-hover:scale-110 transition-transform"
              size={20}
            />
            <input
              type="text"
          ...
          <div className="flex-1 w-full flex items-center bg-black/40 border border-white/10 rounded-2xl px-4 py-3.5 focus-within:border-gold/50 transition-colors relative group hover:bg-black/60">
            ...
            >
              <option value="1 Guest" className="bg-[#111]">
                1 Guest
              </option>
              <option value="2 Guests" className="bg-[#111]">
                2 Guests
              </option>
              <option value="3 Guests" className="bg-[#111]">
                3 Guests
              </option>
              <option value="4+ Guests" className="bg-[#111]">
                4+ Guests
              </option>
            </select>
            <ChevronDown
              size={16}
              className="text-white/50 absolute right-4 pointer-events-none"
            />
          </div>

          <button
            type="submit"
            className="w-full md:w-auto bg-gold bg-[#F1D592] text-[#040C08] px-8 py-3.5 rounded-2xl font-bold flex items-center justify-center transition-all duration-300 175,55,0.2)] shadow-[0_0_30px_rgba(212,175,55,0.4)] hover:-translate-y-0.5 cursor-pointer"
          >
