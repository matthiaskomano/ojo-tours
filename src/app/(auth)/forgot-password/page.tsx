        ...
        <div className="bg-[#0A1A12]/80 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-10 shadow-2xl text-center">
          <Tooltip>
            <TooltipTrigger asChild>
              <Link href="/login">
                <ShieldCheck className="text-gold mx-auto size-12 cursor-pointer mb-6" />
              </Link>
            </TooltipTrigger>
            <TooltipContent>
          ...
          <form action={handleReset} className="space-y-6">
            ...
            {message && <div className="text-gold text-xs py-2">{message}</div>}

            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#F1D592] text-[#040C08] font-bold py-4 rounded-2xl text-xs tracking-widest uppercase transition-all cursor-pointer duration-300 flex items-center justify-center gap-2"
            >
