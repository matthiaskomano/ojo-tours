...
export default function ToursPage() {
  ...
  const filteredTours = tours.filter((tour) => {
    ...
    const matchesSearch =
      tourTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <main className="min-h-screen bg-safari-green selection:bg-gold selection:text-safari-green">
      ...
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden border-b border-white/5">
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-linear-to-t from-safari-green via-safari-green/60 to-transparent" />
        </div>
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-2 md:gap-6 w-full md:w-auto">
            {categories.map((category) => (
              <button
                ...
                className={`relative px-4 py-2 text-xs cursor-pointer tracking-[0.2em] uppercase font-bold transition-colors ${
                ...
                <AnimatePresence mode="popLayout">
                  {paginatedTours.map((pkg) => (
                    ...
                    >
                      ...
                      {/* Tour Content */}
                      <div className="p-8 flex flex-col grow relative z-10 -mt-6 bg-linear-to-b from-transparent to-safari-emerald/50">
                        <div className="mb-4">
                        ...
                        <p className="text-white/60 text-sm leading-loose mb-8 grow">
                          {pkg.description}
                        </p>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center mt-20 gap-4">
                  <button
                    ...
                    className="w-12 h-12 rounded-full border cursor-pointer border-white/10 flex items-center justify-center text-white hover:border-gold hover:text-gold disabled:opacity-30 disabled:hover:border-white/10 disabled:hover:text-white transition-all"
                  >
                    <ChevronLeft size={20} />
                  </button>
